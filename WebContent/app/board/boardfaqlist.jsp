			...
			<tr align="center" valign="middle">
				<td>
					<c:if test="${startPage != 1}">
						<a href="${cp}/board/BoardWrite.bo?page=${startPage-1}&keyword=${keyword}">&lt;</a>
					</c:if>
					<c:forEach begin="${startPage}" end="${endPage}" step="1" var="i">
						<c:choose>
							...
							<c:otherwise>
								<a href="${cp}/board/BoardWrite.bo?page=${i}&keyword=${keyword}">${i}</a>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:if test="${endPage != totalPage }">
						<a href="${cp}/board/BoardWrite.bo?page=${endPage+1}&keyword=${keyword}">&gt;</a>
					</c:if>
				</td>
			</tr>
		</table>  --%>
